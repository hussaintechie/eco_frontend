...
const ProfilePage = () => {
  ...
  return (
    <div
      className={`min-h-screen ${theme.gradient} transition-colors duration-500 pb-10 relative`}
    >
      {/* --- Top Banner --- */}
      <div
        className={`${theme.bannerGradient} h-48 relative overflow-hidden rounded-b-[2rem] shadow-lg`}
      >
  ...
  `}
        ...
        <div className="mt-6 space-y-4">
          {menuItems.map((item, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`p-3 rounded-lg ${theme.accent} ${theme.primaryText}`}
                >
                  <item.icon size={22} />
                </div>
                <div>
          ...
          <button
            className="w-full bg-white p-4 rounded-xl shadow-sm flex items-center gap-4 text-red-500"
