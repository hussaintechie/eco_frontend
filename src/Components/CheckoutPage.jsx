...
const dates = [
  { label: "Today", sub: "2 Dec" },
  { label: "Tomorrow", sub: "3 Dec" },
  { label: "Thursday", sub: "4 Dec" },
  { label: "Friday", sub: "5 Dec" },
];

const fallbackTimeSlots = [
  { time: "1:00 PM - 2:00 PM", available: false },
  { time: "2:30 PM - 3:30 PM", available: false },
  { time: "4:00 PM - 5:00 PM", available: false },
  { time: "5:30 PM - 6:30 PM", available: true },
...
const CheckoutPage = () => {
  ...
  const [slots, setSlots] = useState([]); // from backend
      ...
      <div className="bg-white sticky top-0 z-20 shadow-sm">
        ...
        <div className={`${theme.light} py-2 text-center border-t border-b ${theme.border}`}>
          <p className={`text-xs font-bold ${theme.primaryText}`}>
            🎉 Congrats! You're saving ₹31 on this order
          </p>
        </div>
      </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 hidden md:block">
                  We will pack your items in a reusable bag.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ${
                  isBagOpted ? theme.primary : "bg-slate-300"
                }`}
              >
                <div
                  className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-300 ${
                    isBagOpted ? "translate-x-6" : "translate-x-0"
                  }`}
                />
              </button>
            </div>
      ...
      {showSlotModal && (
        <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200 p-0 md:p-6">
          <div className="bg-white w-full md:max-w-lg rounded-t-3xl md:rounded-3xl p-5 md:p-8 h-[70vh] md:h-auto overflow-y-auto relative animate-in slide-in-from-bottom duration-300 md:shadow-2xl">
            ...
            {/* Time slots from backend if available, else fallback grid */}
            {slots.length > 0 ? (
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left py-3 px-3 rounded-lg border text-sm font-medium transition-all ${
                        isSelected
                          ? `${theme.light} ${theme.border} ${theme.primaryText} ring-1 ring-inset ${theme.primaryText}`
              ...
              <div className="grid grid-cols-2 gap-3 mb-24 md:mb-8">
                {fallbackTimeSlots.map((slot, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-4 px-2 rounded-lg border text-sm font-medium transition-all ${
                        !slot.available
